...
local function font(val)
	...
	-- return "${font Mono:size="..val.."}"
	return "${font DejaVu Sans Mono:size="..(val*coef_zoom_font).."}"
end
	...
	if not cmus_info.duration or ((t-last_read)>(cmus_info.duration-cmus_info.position)) or (t>(refresh+last_read)) then
		...
		while true do
			...
			elseif string.find(line,"^tag artist") then
				-- url: http://www.lua.org/pil/20.2.html
--				cmus_info.artist = string.match(line,"[%s%w%d]*",12)
	...
	string = string.."\n$hr\n"
	if cmus_info.status then
		string=string..cmus_info.status.." - "
	end
	...
	-- string=string .. font(18)
	string=string .. font(10 * (1.0/coef_zoom_font))
